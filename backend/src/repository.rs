...
pub async fn get_user_id_by_username_tx(
...
pub async fn get_user_passkeys(
    ...
    let creds = sqlx::query!(
        "SELECT passkey_json FROM credentials WHERE user_id = $1",
        user_id
    )
    .fetch_all(pool)
    .await?;
...
pub async fn get_user_id_by_cred_id(
    pool: &PgPool,
    cred_id_b64: &str,
) -> Result<Option<Uuid>, Error> {
    let rec = sqlx::query!(
        "SELECT user_id FROM credentials WHERE cred_id = $1",
        cred_id_b64
    )
    .fetch_optional(pool)
    .await?;
...
pub async fn list_credentials_info(
    pool: &PgPool,
    user_id: Uuid,
) -> Result<Vec<CredentialInfo>, Error> {
    let creds = sqlx::query!(
